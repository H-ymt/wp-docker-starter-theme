...
if (
  class_exists("Ai1wm_Backup_WP_CLI_Base") &&
  !class_exists("Ai1wm_Backup_WP_CLI_Command")
) {
  class Ai1wm_Backup_WP_CLI_Command extends Ai1wm_Backup_WP_CLI_Base
  {
    ...
    {
      $params = $this->run_backup($this->build_export_params($args, $assoc_args));

      WP_CLI::log(
        sprintf(__("Backup location: %s", AI1WM_PLUGIN_NAME), ai1wm_backup_path($params)),
      );
    }
    ...
    public function list_backups(array $args, array $assoc_args)
    {
      ...
      foreach ($model->get_files() as $backup) {
        $backups->addRow([
          ...
          "date" => sprintf(
            __("%s ago", AI1WM_PLUGIN_NAME),
            human_time_diff($backup["mtime"]),
          ),
     ...
     */
    public function browse_backup($args = [], $assoc_args = [])
    {
      $params = [
        "cli_args" => $assoc_args,
      ...
      if (!isset($args[0])) {
        WP_CLI::error_multi_line([
          __(
            "A backup name must be provided in order to proceed with the list backup files process.",
            AI1WM_PLUGIN_NAME,
          ),
          __(
            "Example: wp ai1wm backup_files migration-wp-20170913-095743-931.wpress",
            AI1WM_PLUGIN_NAME,
          ),
        ]);
        exit();
      ...
      if (!is_file(ai1wm_backup_path(["archive" => $args[0]]))) {
        WP_CLI::error_multi_line([
          __(
            "The backup file could not be located in wp-content/ai1wm-backups folder.",
            AI1WM_PLUGIN_NAME,
          ),
     ...
     */
    public function extract_backup($args = [], $assoc_args = [])
    {
      $params = [
        "cli_args" => $assoc_args,
        "secret_key" => get_option(AI1WM_SECRET_KEY, false),
      ...
      if (!isset($args[0])) {
        WP_CLI::error_multi_line([
          __(
            "A backup name must be provided in order to proceed with the extract process.",
            AI1WM_PLUGIN_NAME,
          ),
          __(
            "Example: wp ai1wm backup-extract migration-wp-20170913-095743-931.wpress",
            AI1WM_PLUGIN_NAME,
          ),
        ]);
        exit();
      ...
      if (!isset($assoc_args["extract-path"])) {
        WP_CLI::error_multi_line([
          __(
            "A backup extract path must be provided and writable in order to proceed with the extract process.",
            AI1WM_PLUGIN_NAME,
          ),
          __(
            "Example: wp ai1wm backup-extract migration-wp-20170913-095743-931.wpress --extract-path=/tmp",
            AI1WM_PLUGIN_NAME,
          ),
        ]);
        exit();
      ...
      if (!is_writable($params["extract_path"])) {
        WP_CLI::error_multi_line([
          __(
            "The extract folder is not writable. Check that directory exists and writable.",
            AI1WM_PLUGIN_NAME,
          ),
        ]);
        exit();
      ...
      try {
        ...
        WP_CLI::error(
          sprintf(__("Unable to extract files: %s", AI1WM_PLUGIN_NAME), $e->getMessage()),
        );
        exit();
      }
    }
    ...
    public function restore($args = [], $assoc_args = [])
    {
      ...
      if (!isset($args[0])) {
        WP_CLI::error_multi_line([
          __(
            "A backup name must be provided in order to proceed with the restore process.",
            AI1WM_PLUGIN_NAME,
          ),
          __(
            "Example: wp ai1wm restore migration-wp-20170913-095743-931.wpress",
            AI1WM_PLUGIN_NAME,
          ),
        ]);
        exit();
