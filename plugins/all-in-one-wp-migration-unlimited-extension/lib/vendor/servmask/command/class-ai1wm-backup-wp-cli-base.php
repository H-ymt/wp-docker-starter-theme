...
if (defined("WP_CLI") && !class_exists("Ai1wm_Backup_WP_CLI_Base")) {
  abstract class Ai1wm_Backup_WP_CLI_Base extends WP_CLI_Command
  {
    public function __construct()
    {
      if (!defined("AI1WM_PLUGIN_NAME")) {
        WP_CLI::error_multi_line([
          __(
            "Extension requires All-in-One WP Migration plugin to be activated. ",
            "all-in-one-wp-migration",
          ),
          __(
            "You can get a copy of it here: https://wordpress.org/plugins/all-in-one-wp-migration/",
            "all-in-one-wp-migration",
          ),
        ]);
        exit();
      ...
      if (!is_dir(AI1WM_STORAGE_PATH)) {
        if (!mkdir(AI1WM_STORAGE_PATH)) {
          WP_CLI::error_multi_line([
            ...
            __(
              "You will need to create this folder and grant it read/write/execute permissions (0777) for the All-in-One WP Migration plugin to function properly.",
              AI1WM_PLUGIN_NAME,
            ),
          ]);
          exit();
     ...
     */
    protected function build_export_params($args = [], $assoc_args = [])
    {
      $params = [
        "cli_args" => $assoc_args,
      ...
      if (isset($assoc_args["password"])) {
        if (function_exists("ai1wm_can_encrypt") && ai1wm_can_encrypt()) {
          if ($assoc_args["password"] === true || empty($assoc_args["password"])) {
            $assoc_args["password"] = readline(
              "Please enter a password to protect this backup: ",
            );
          }

          if (empty($assoc_args["password"])) {
            WP_CLI::error(
              __("Encryption password must not be empty.", AI1WM_PLUGIN_NAME),
            );
          }
          ...
          WP_CLI::error(
            __('Your system doesn\'t support encryption.', AI1WM_PLUGIN_NAME),
          );
        }
      }
      ...
      if (isset($assoc_args["exclude-database"])) {
        ...
        if (
          $assoc_args["exclude-tables"] === true ||
          empty($assoc_args["exclude-tables"])
        ) {
          ...
          while (
            $table = trim(
              readline(
                "Enter table name to exclude from backup (q=quit, empty=continue): ",
              ),
            )
          ) {
            switch ($table) {
    ...
    protected function run_backup($params)
    {
      ...
      try {
        ...
        WP_CLI::error(
          __(sprintf("Unable to backup: %s", $e->getMessage()), AI1WM_PLUGIN_NAME),
        );
      }

      WP_CLI::success(__("Backup complete.", AI1WM_PLUGIN_NAME));
      WP_CLI::log(
        sprintf(__("Backup file: %s", AI1WM_PLUGIN_NAME), ai1wm_archive_name($params)),
      );
    ...
    protected function run_restore($params)
    {
      ...
      try {
        ...
        WP_CLI::error(
          __(sprintf("Unable to import: %s", $e->getMessage()), AI1WM_PLUGIN_NAME),
        );
      }
