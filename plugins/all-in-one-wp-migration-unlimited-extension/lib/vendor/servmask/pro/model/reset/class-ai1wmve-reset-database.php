...
class Ai1wmve_Reset_Database
{
  ...
  public static function execute($params)
  {
    ...
    if (
      ...
      Ai1wm_Status::done(
        __("Reset Successful"),
        __(
          'The reset of your site\'s database has been successfully completed. WordPress has been restored to its original state, similar to a new installation. Please remember, this action has removed all content, settings, and user data. Proceed to set up your site again or restore from a backup if necessary.',
          AI1WM_PLUGIN_NAME,
        ),
      );
      exit();
  ...
  protected function backup_options()
  {
    ...
    $ai1wm_options = $wpdb->get_results(
      sprintf(
        'SELECT `option_name`, `option_value` FROM `%s` WHERE `option_name` LIKE \'ai1wm%%\'',
  ...
  protected function restore_data()
  {
    ...
    $guess_url =
      wp_guess_url() !== "http:" ? wp_guess_url() : $backup["blog"]["site_url"];
    ...
    if (is_multisite()) {
      ...
      $wpdb->insert(ai1wm_table_prefix() . "site", [
        "domain" => $domain,
        "path" => $path,
      ]);
    ...
    foreach ($backup["options"] as $name => $value) {
      $wpdb->insert(ai1wm_table_prefix() . "options", [
        "option_name" => $name,
        "option_value" => $value,
      ]);
    }
