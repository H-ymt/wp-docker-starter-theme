...
class Ai1wmve_Reset_Themes
{
  ...
  public static function execute($params)
  {
    ...
    if (
      ...
      Ai1wm_Status::done(
        __("Reset Successful"),
        __(
          'Your theme has been successfully reset. All themes except the default WordPress theme have been removed, and the default theme is now active. This process has cleared all customizations and settings related to other themes. You can now start fresh with theme setup or install a new theme to customize your site\'s appearance.',
          AI1WM_PLUGIN_NAME,
        ),
      );
      exit();
  ...
  protected static function reset_theme_options($blog_id = null)
  {
    ...
    $wpdb->query(
      sprintf(
        "DELETE FROM `%s` WHERE `option_name` LIKE 'mods\_%%' OR `option_name` LIKE 'theme\_mods\_%%'",
        ai1wm_table_prefix($blog_id) . "options",
      ),
    );
  }
  ...
  {
    $api = themes_api("theme_information", ["slug" => $theme]);
    if (!is_wp_error($api)) {
  ...
  {
    // Create theme files
    Ai1wm_Directory::create(
      get_theme_root() . DIRECTORY_SEPARATOR . AI1WMVE_RESET_THEME_NAME,
    );
    Ai1wm_File::create(
