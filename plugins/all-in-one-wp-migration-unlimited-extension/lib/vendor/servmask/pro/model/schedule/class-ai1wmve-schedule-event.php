...
if (!class_exists("Ai1wmve_Schedule_Event")) {
  class Ai1wmve_Schedule_Event
  {
    ...
    {
      if (!is_array($data)) {
        throw new Ai1wmve_Schedules_Exception(
          __("Event data must be array", AI1WM_PLUGIN_NAME),
        );
      }
      ...
      foreach ($data as $name => $value) {
        if (property_exists($this, $name)) {
          if (method_exists($this, $name)) {
            ...
            $this->{$name} =
              is_array($this->{$name}) && is_array($value)
      ...
      switch ($this->schedule["interval"]) {
        case self::INTERVAL_HOURLY:
          return $this->incremental
            ? __("Continuous", AI1WM_PLUGIN_NAME)
            : __("Per hour", AI1WM_PLUGIN_NAME);

        case self::INTERVAL_DAILY:
          return $this->incremental
            ? __("Once per day", AI1WM_PLUGIN_NAME)
            : __("Per day", AI1WM_PLUGIN_NAME);
    ...
    protected function timestamp()
    {
      ...
      $cron_time->setTime(
        intval($this->schedule["hour"]),
        intval($this->schedule["minute"]),
      );

      if (
        $this->schedule["interval"] === self::INTERVAL_MONTHLY &&
        is_numeric($this->schedule["day"])
      ) {
        $cron_time->setDate(
          $cron_time->format("Y"),
          $cron_time->format("m"),
    ...
    protected function recurrence()
    {
      switch ($this->schedule["interval"]) {
        ...
        default:
          throw new Ai1wmve_Schedules_Exception(
            __("Undefined schedule interval!", AI1WM_PLUGIN_NAME),
          );
      }
    }
    ...
    {
      add_filter("ai1wm_notification_ok_toggle", [
        $this,
        "is_success_notification_enabled",
      ]);
      add_filter("ai1wm_notification_ok_email", [$this, "notification_email"]);
      ...
      $notification =
        sprintf(
          __("<p>Your site %s was successfully exported.</p>", AI1WM_PLUGIN_NAME),
          site_url(),
        ) .
        sprintf(__("<p>Date: %s</p>", AI1WM_PLUGIN_NAME), date_i18n("r")) .
        sprintf(
          __("<p>Backup file: %s</p>", AI1WM_PLUGIN_NAME),
      ...
      Ai1wm_Notification::ok(
        sprintf(
          __("✅ Scheduled event has completed (%s)", AI1WM_PLUGIN_NAME),
    ...
    {
      add_filter("ai1wm_notification_error_toggle", [
        $this,
        "is_failed_notification_enabled",
      ]);
      add_filter("ai1wm_notification_error_email", [$this, "notification_email"]);

      $notification =
        sprintf(__("<p>Your site %s export failed.</p>", AI1WM_PLUGIN_NAME), site_url()) .
        sprintf(__("<p>Date: %s</p>", AI1WM_PLUGIN_NAME), date_i18n("r"));

      if ($error) {
        $notification .= sprintf(
          __("<p>Error message: %s</p>", AI1WM_PLUGIN_NAME),
          $error,
        );
      }
      ...
      Ai1wm_Notification::error(
        sprintf(
          __("❌ Scheduled event has failed (%s)", AI1WM_PLUGIN_NAME),
          parse_url(site_url(), PHP_URL_HOST) . parse_url(site_url(), PHP_URL_PATH),
        ),
        $notification,
      );
    ...
    {
      return $this->notification["reminder"] === static::REMINDER_SUCCESS &&
        $this->notification["status"] === static::STATUS_ENABLED;
    }

    public function is_failed_notification_enabled()
    {
      return $this->notification["reminder"] === static::REMINDER_FAILED &&
        $this->notification["status"] === static::STATUS_ENABLED;
    }
    ...
    protected function params()
    {
      $params = [
        ...
        "event_retention_size" =>
          (int) $this->retention["total"] . trim($this->retention["total_unit"]),
