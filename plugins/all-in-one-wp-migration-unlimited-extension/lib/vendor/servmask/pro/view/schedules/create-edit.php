	...
	<div class="ai1wm-row">
		<div class="ai1wm-left">
			<div class="ai1wm-holder" id="ai1wmve-schedule-event-form-component">
				...
				<schedule-event inline-template :event='<?php echo $event->to_json(); ?>' :advanced-options='<?php echo json_encode(
  $event->advanced_options(),
); ?>' :global-increment="<?php echo defined("AI1WM_INCREMENTAL_PATH")
  ? "true"
  : "false"; ?>">
					<form method="post" action="<?php echo esc_url(
       admin_url("admin-post.php?action=ai1wm_schedule_event_save"),
     ); ?>" id="ai1wmve-schedule-event-form" class="ai1wm-clear">
						...
						<div class="ai1wm-event-fieldset">
							...
							<div class="ai1wm-event-row">
								<div class="ai1wm-event-field">
									<label class="ai1wm-event-label" for="ai1wm-event-title"><?php _e(
           "Title",
           AI1WM_PLUGIN_NAME,
         ); ?></label>
									<input type="text" class="ai1wm-event-input" id="ai1wm-event-title" name="title" v-model="form.title" placeholder="<?php _e(
           "Event title here",
           AI1WM_PLUGIN_NAME,
         ); ?>" required/>
								</div>
								<div class="ai1wm-event-field">
									<label class="ai1wm-event-label" for="ai1wm-event-type"><?php _e(
           "Event type",
           AI1WM_PLUGIN_NAME,
         ); ?></label>
							...
							<div class="ai1wm-event-row" v-if="advancedTypeOptions.length">
								<div class="ai1wm-event-field">
									<label class="ai1wm-event-label"><?php _e(
           "Advanced options",
           AI1WM_PLUGIN_NAME,
         ); ?></label>
								...
								<div class="ai1wm-event-field ai1wm-encrypt-backups-container">
									...
									<div class="ai1wm-encrypt-backups-passwords-toggle" v-if="encrypted">
										<div class="ai1wm-encrypt-backups-passwords-container">
											<toggle-password name="password" placeholder="<?php _e(
             "Enter a password",
             AI1WM_PLUGIN_NAME,
           ); ?>" class-name="ai1wm-event-input ai1wm-event-input-small" v-model="form.password"></toggle-password>
											<toggle-password name="password_confirmation" placeholder="<?php _e(
             "Repeat the password",
             AI1WM_PLUGIN_NAME,
           ); ?>" class-name="ai1wm-event-input ai1wm-event-input-small" v-model="password" :error="passwordConfirmed ? null : '<?php _e(
							...
							<div class="ai1wm-event-row" v-if="form.type === '<?php echo Ai1wmve_Schedule_Event::TYPE_EXPORT; ?>'">
								<div class="ai1wm-event-field ai1wm-event-field-row" id="ai1wmve-db-table-excluder">
									<label for="ai1wmve-exclude_db_tables" v-show="showDbExcluder">
										...
										<?php _e(
            "Do <strong>not</strong> include the selected database tables",
            AI1WM_PLUGIN_NAME,
          ); ?>
									</label>
									<db-tables v-show="showDbExcluder" :value="this.excludedDbTables" :db-tables='<?php echo json_encode(
           $tables,
           JSON_HEX_APOS,
         ); ?>'></db-tables>
								</div>
						...
						<div class="ai1wm-event-fieldset" v-if="form.type">
							...
							<div class="ai1wm-event-row">
								<div class="ai1wm-event-field">
									<label class="ai1wm-event-label" for="ai1wm-event-storage"><?php _e(
           "Storage",
           AI1WM_PLUGIN_NAME,
         ); ?></label>
									<select class="ai1wm-event-input" id="ai1wm-event-storage" name="storage" v-model="form.storage" required>
										...
										<?php foreach (apply_filters("ai1wmve_export_buttons_schedules", []) as $button): ?>
											<?php echo $button; ?>
										<?php endforeach; ?>
									</select>
								</div>
							...
							<div class="ai1wm-event-row" v-if="storageLink">
								<a :href="storageLink" v-if="this.isServMaskLink" target="_blank"
									v-html="'<?php _e(
           "To use <strong>%s</strong> storage, purchase it here.",
           AI1WM_PLUGIN_NAME,
         ); ?>'.replace('%s', storageName)"></a>
								<a :href="storageLink" v-else
									v-html="'<?php _e(
           "To use <strong>%s</strong> storage, you need to configure it first.",
           AI1WM_PLUGIN_NAME,
         ); ?>'.replace('%s', storageName)"></a>
							</div>
						...
						<div class="ai1wm-event-fieldset" v-if="form.type">
							...
							<div class="ai1wm-event-row">
								<div class="ai1wm-event-field">
									<label class="ai1wm-event-label" for="ai1wm-event-schedule-interval"><?php _e(
           "Interval",
           AI1WM_PLUGIN_NAME,
         ); ?></label>
            ...
            Ai1wmve_Schedule_Event::INTERVAL_HOURLY,
          ); ?>" v-text="form.incremental ? '<?php _e(
  "Continuous",
  AI1WM_PLUGIN_NAME,
); ?>' : '<?php _e("Hourly", AI1WM_PLUGIN_NAME); ?>'"></option>
										<option value="<?php echo esc_attr(
            Ai1wmve_Schedule_Event::INTERVAL_DAILY,
          ); ?>" v-text="form.incremental ? '<?php _e(
  "Once per day",
  AI1WM_PLUGIN_NAME,
); ?>' : '<?php _e("Daily", AI1WM_PLUGIN_NAME); ?>'"></option>
										<option value="<?php echo esc_attr(Ai1wmve_Schedule_Event::INTERVAL_WEEKLY); ?>"><?php _e(
  "Weekly",
  AI1WM_PLUGIN_NAME,
); ?></option>
										<option value="<?php echo esc_attr(
            Ai1wmve_Schedule_Event::INTERVAL_MONTHLY,
          ); ?>"><?php _e("Monthly", AI1WM_PLUGIN_NAME); ?></option>
										<option value="<?php echo esc_attr(Ai1wmve_Schedule_Event::INTERVAL_N_HOUR); ?>"><?php _e(
  "N Hour",
  AI1WM_PLUGIN_NAME,
); ?></option>
										<option value="<?php echo esc_attr(Ai1wmve_Schedule_Event::INTERVAL_N_DAYS); ?>"><?php _e(
  "N Days",
  AI1WM_PLUGIN_NAME,
); ?></option>
									</select>
								...
								<div class="ai1wm-event-field ai1wm-event-field-nested">
									<div class="ai1wm-event-field" v-if="form.schedule.interval === '<?php echo Ai1wmve_Schedule_Event::INTERVAL_WEEKLY; ?>'">
										<label class="ai1wm-event-label" for="ai1wm-event-schedule-weekday"><?php _e(
            "Day",
            AI1WM_PLUGIN_NAME,
          ); ?></label>
									...
									<div class="ai1wm-event-field" v-else-if="form.schedule.interval === '<?php echo Ai1wmve_Schedule_Event::INTERVAL_MONTHLY; ?>'">
										<label class="ai1wm-event-label" for="ai1wm-event-schedule-day"><?php _e(
            "Day",
            AI1WM_PLUGIN_NAME,
          ); ?></label>
										<select class="ai1wm-event-input" id="ai1wm-event-schedule-day" v-model="form.schedule.day" name="schedule[day]" required>
											...
											<?php foreach (range(1, 28) as $day): ?>
												<option value="<?php echo $day; ?>"><?php echo date_i18n(
  "d",
  mktime(null, null, null, null, $day),
); ?></option>
											<?php endforeach; ?>
										</select>
									</div>
									...
									<div class="ai1wm-event-field" v-if="form.schedule.interval === '<?php echo Ai1wmve_Schedule_Event::INTERVAL_N_HOUR; ?>' || form.schedule.interval === '<?php echo Ai1wmve_Schedule_Event::INTERVAL_N_DAYS; ?>'">
										<label class="ai1wm-event-label" for="ai1wm-event-schedule-n" v-text="form.schedule.interval === '<?php echo Ai1wmve_Schedule_Event::INTERVAL_N_HOUR; ?>' ? '<?php _e(
  "N Hour",
  AI1WM_PLUGIN_NAME,
); ?>' : '<?php _e("N Days", AI1WM_PLUGIN_NAME); ?>'"></label>
										<input type="number" class="ai1wm-event-input" id="ai1wm-event-schedule-n" name="schedule[n]" v-model="form.schedule.n" :placeholder="form.schedule.interval === '<?php echo Ai1wmve_Schedule_Event::INTERVAL_N_HOUR; ?>' ? '<?php _e(
  "Hours",
  AI1WM_PLUGIN_NAME,
); ?>' : '<?php _e("Days", AI1WM_PLUGIN_NAME); ?>'" required />
									</div>
									...
									<div class="ai1wm-event-field" v-if="form.schedule.interval && form.schedule.interval !== '<?php echo Ai1wmve_Schedule_Event::INTERVAL_N_HOUR; ?>' && form.schedule.interval !== '<?php echo Ai1wmve_Schedule_Event::INTERVAL_HOURLY; ?>'">
										<label class="ai1wm-event-label" for="ai1wm-event-schedule-hour"><?php _e(
            "Hour",
            AI1WM_PLUGIN_NAME,
          ); ?></label>
										<select class="ai1wm-event-input" id="ai1wm-event-schedule-hour" v-model="form.schedule.hour" name="schedule[hour]" required>
											...
											<?php foreach (range(0, 23) as $hour): ?>
												<option value="<?php echo $hour; ?>"><?php echo date_i18n(
  "g a",
  mktime($hour),
); ?></option>
											<?php endforeach; ?>
									...
									<div class="ai1wm-event-field" v-if="! form.incremental && form.schedule.interval">
										<label class="ai1wm-event-label" for="ai1wm-event-schedule-minute"><?php _e(
            "Minute",
            AI1WM_PLUGIN_NAME,
          ); ?></label>
										<select class="ai1wm-event-input" id="ai1wm-event-schedule-minute" v-model="form.schedule.minute" name="schedule[minute]" required>
											...
											<?php foreach (range(0, 59) as $minute): ?>
												<option value="<?php echo $minute; ?>"><?php echo date_i18n(
  "i",
  mktime(null, $minute),
); ?></option>
											<?php endforeach; ?>
						...
						<div class="ai1wm-event-fieldset" v-if="form.type">
							...
							<div class="ai1wm-event-row">
								<div class="ai1wm-event-field">
									<label class="ai1wm-event-label" for="ai1wm-event-notification-reminder"><?php _e(
           "Reminder",
           AI1WM_PLUGIN_NAME,
         ); ?></label>
									<select class="ai1wm-event-input" id="ai1wm-event-notification-reminder" v-model="form.notification.reminder" name="notification[reminder]" required>
										...
										<option value="<?php echo esc_attr(Ai1wmve_Schedule_Event::REMINDER_NONE); ?>"><?php _e(
  "No notification",
  AI1WM_PLUGIN_NAME,
); ?></option>
										<option value="<?php echo esc_attr(
            Ai1wmve_Schedule_Event::REMINDER_SUCCESS,
          ); ?>"><?php _e("Notify on success", AI1WM_PLUGIN_NAME); ?></option>
										<option value="<?php echo esc_attr(Ai1wmve_Schedule_Event::REMINDER_FAILED); ?>"><?php _e(
  "Notify on failure",
  AI1WM_PLUGIN_NAME,
); ?></option>
									</select>
								...
								<div class="ai1wm-event-field">
									<label class="ai1wm-event-label" for="ai1wm-event-notification-status"><?php _e(
           "Status",
           AI1WM_PLUGIN_NAME,
         ); ?></label>
									<select class="ai1wm-event-input" id="ai1wm-event-notification-status" v-model="form.notification.status" name="notification[status]" :disabled="!form.notification.reminder || form.notification.reminder === '<?php echo esc_attr(
           Ai1wmve_Schedule_Event::REMINDER_NONE,
         ); ?>'">
										<option value="" disabled><?php _e("Select status", AI1WM_PLUGIN_NAME); ?></option>
										<option value="<?php echo esc_attr(Ai1wmve_Schedule_Event::STATUS_ENABLED); ?>"><?php _e(
  "Enabled",
  AI1WM_PLUGIN_NAME,
); ?></option>
										<option value="<?php echo esc_attr(Ai1wmve_Schedule_Event::STATUS_DISABLED); ?>"><?php _e(
  "Disabled",
  AI1WM_PLUGIN_NAME,
); ?></option>
									</select>
								...
								<div class="ai1wm-event-field">
									<label class="ai1wm-event-label" for="ai1wm-event-notification-email"><?php _e(
           "Email",
           AI1WM_PLUGIN_NAME,
         ); ?></label>
									<input type="text" class="ai1wm-event-input" id="ai1wm-event-notification-email" v-model="form.notification.email" name="notification[email]" placeholder="<?php _e(
           "Your email here",
           AI1WM_PLUGIN_NAME,
         ); ?>" :disabled="!form.notification.reminder || form.notification.reminder === '<?php echo esc_attr(
  Ai1wmve_Schedule_Event::REMINDER_NONE,
); ?>'" />
								</div>
							</div>
						...
						<div class="ai1wm-event-fieldset" v-if="form.type">
							...
							<div class="ai1wm-event-row">
								<div class="ai1wm-event-field">
									<label class="ai1wm-event-label" for="ai1wm-event-status"><?php _e(
           "Status",
           AI1WM_PLUGIN_NAME,
         ); ?></label>
