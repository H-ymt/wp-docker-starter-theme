    ...
    if ($filters = ai1wm_get_filters("ai1wm_reset")) {
      while ($hooks = current($filters)) {
        if (intval($params["priority"]) === key($filters)) {
          foreach ($hooks as $hook) {
            try {
              ...
              if (defined("WP_CLI")) {
                WP_CLI::error(
                  sprintf(
                    __("Unable to Proceed: %s", AI1WM_PLUGIN_NAME),
                    $e->getMessage(),
                  ),
                );
              } else {
                Ai1wm_Status::error(
                  __("Unable to Proceed", AI1WM_PLUGIN_NAME),
                  $e->getMessage(),
                );
          ...
          if (
            $completed === false ||
            (($next = next($filters)) && ($params["priority"] = key($filters)))
          ) {
            if (defined("WP_CLI")) {
            ...
            wp_remote_request(
              ...
              [
                ...
                "headers" => apply_filters("ai1wm_http_reset_headers", []),
