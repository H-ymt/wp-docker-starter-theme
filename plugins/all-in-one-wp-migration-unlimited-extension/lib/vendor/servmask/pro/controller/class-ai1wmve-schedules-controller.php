  ...
  class Ai1wmve_Schedules_Controller
  {
    ...
    public static function delete()
    {
      ...
      if (!isset($params["event_id"])) {
        ai1wm_json_response(["errors" => __("Unable to find event", AI1WM_PLUGIN_NAME)]);
        exit();
      }
    ...
    public static function save($params = [])
    {
      ...
      Ai1wm_Message::flash(
        "schedules",
        __("Your changes have been saved.", AI1WM_PLUGIN_NAME),
      );
    ...
    {
      $buttons = [];
      foreach (apply_filters("ai1wm_export_buttons", []) as $button) {
    ...
    public static function run($event_id)
    {
      ...
      return file_put_contents(
        AI1WM_STORAGE_PATH . DIRECTORY_SEPARATOR . "cron.log",
        "Event not found " . $event_id . PHP_EOL,
        FILE_APPEND,
      );
    }
    ...
    public static function log_failed($params)
    {
      if (isset($params["event_id"])) {
        ...
        $events = new Ai1wmve_Schedule_Events();
        if ($event = $events->find($event_id)) {
          $message = isset($params["error_message"])
            ? $params["error_message"]
            : __("Unknown cron error", AI1WM_PLUGIN_NAME);
